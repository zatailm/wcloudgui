...
os.environ["QT_SCALE_FACTOR"] = "0.90"
...
class ThreadManager(QObject):
    ...
    def stop_all_threads(self, wait_timeout=1000):
        ...
        active_thread_count = self.active_threads
        logger.info(f"Stopping all active threads ({active_thread_count})")
        self._mutex.lock()
            ...
            try:
                if thread and thread.isRunning():
                    ...
                    if not thread.wait(wait_timeout):
                        logger.warning(f"Thread {thread.objectName()} not responding, terminating...")
                        thread.terminate()
                        thread.wait(500)
        ...
        return {
            "total": len(threads_to_stop),
            "stopped": len(stopped_threads),
            "failed": len(failed_to_stop),
            "remaining": self.active_threads
        }
class ThreadFactory:
    ...
    def generate_wordcloud(self, text_data, params):
        ...
        try:
            ...
            if self.resource_controller:
                self.font_map = self.resource_controller.res_get_available_fonts()
            params['font_map'] = self.font_map
            if 'colormap' not in params and self.resource_controller:
                params['colormap'] = self.resource_controller.res_get_palette_names()[0] if self.resource_controller.res_get_palette_names() else 'viridis'
        ...
        try:
            ...
            if title_text:
                ...
                Buat image baru dengan ruang untuk title
                    Gunakan:
                    total_height = int(wc_image.height * 1.15) 
                    1.15 = faktor untuk menambahkan 15% ruang vertical untuk title
                    dan gunakan:
                    combined_image.paste(wc_image, (0, int(wc_image.height * 0.15)))  
                    untuk menempatkan wordcloud di bawah judul dengan Faktor 0.15
                    jika tidak menggunakan variabel total_height.
                """
                total_height = int(wc_image.height * 1.15) 
...
class SentimentAnalysisResultDialog(QDialog):
    ...
    def setup_ui(self):
        ...
        # Sentiment Label with colored background
        ...
        # Prepare table data - different for each method
        table_data = []
        # Main score for all methods
        table_data.append((main_score_type, f"{self.result['compound_score']:.4f}"))
        # Component scores based on method
        if self.sentiment_mode in ["VADER", "VADER (Custom Lexicon)"]:
            # VADER - include all component scores
            ...
            # Flair - exclude neutral score
            table_data.append(("Positive Score", f"{self.result['positive_score']:.4f}"))
            table_data.append(("Negative Score", f"{self.result['negative_score']:.4f}"))
        # Subjectivity only for TextBlob
    ...
    def get_interpretation(self) -> str:
        ...
        if self.sentiment_mode in ["VADER", "VADER (Custom Lexicon)"]:
            ...
            # subj_text = "highly subjective" if subj > 0.7 else "relatively objective" #nilai awal 0.5
    ...
    #                 writer.writerow(['Compound/Confidence Score', f"{self.result['compound_score']:.4f}"])
        ...
        if file_path:
            try:
                with open(file_path, 'w', newline='', encoding='utf-8') as f:
                    ...
                    writer.writerow(['Analysis Method', self.sentiment_mode])
                    writer.writerow(['Sentiment', self.result["sentiment_label"]])
                    # Gunakan nama score yang sesuai dengan metode
                    if self.sentiment_mode in ["VADER", "VADER (Custom Lexicon)"]:
...
class TopicAnalysisResultDialog(QDialog):
    ...
    def setup_ui(self):
        ...
        for i, topic in enumerate(topics):
            ...
            # Top terms
            terms = ", ".join(topic['terms'][:5])  # Show top 5 terms
    ...
    def find_related_terms(self, target_term: str, topics: list) -> list:
        ...
        for topic in topics:
            ...
            if target_term in terms:
                related.update(terms[:5])  # Add top 5 terms from the topic
...
class KeywordExtractionResultDialog(QDialog):
    ...
    def setup_ui(self):
        ...
        for i, result in enumerate(self.results):
            ...
            # Score - Note: for YAKE, lower scores are better
            score = result['score']
            if self.method == 'YAKE':
    ...
    def run(self):
        try:
            if self.method == 'tfidf':
                result = self._extract_tfidf()
            elif self.method == 'yake':
                result = self._extract_yake()
            elif self.method == 'rake':
                result = self._extract_rake()
            else:
...
class WordCloudGeneratorThread(QThread):
    ...
    def run(self):
        try:
            ...
            self.progress_updated.emit(10)
            from matplotlib.font_manager import findfont, FontProperties
            default_font_path = findfont(FontProperties())
            font_path = None
            if self.params.get('font_choice') and self.params['font_choice'] != "Default":
                font_path = self.params.get('font_map', {}).get(self.params['font_choice'])
            if not font_path:
            ...
            if nltk_stopwords is not None:
                combined_stopwords = nltk_stopwords.union(user_stopwords) if user_stopwords else nltk_stopwords
            else:
                combined_stopwords = user_stopwords
            wc_params = {
                'width': 1000,
                'height': 600,
...
class TopicAnalysisTab(QWidget):
    ...
    # def show_results_dialog(self, method, results):
    #     """Show topic analysis results in a dialog"""
    ...
    def show_keyword_dialog(self, method: str, results: list):
        ...
        # Tidak perlu parameter num_keywords, karena kita bisa menggunakan len(results)
...
class AppUI(QMainWindow):
    ...
    def _on_system_fonts_loaded(self, fonts_dict):
        ...
        try:
            ...
            if hasattr(self, 'resource_controller') and hasattr(self.resource_controller, '_cached_fonts'):
    ...
    def stop_all_processes(self):
        """Enhanced process termination including sentiment analysis"""
        ...
        try:
            special_threads = []
            special_thread_names = [
                'file_loader_thread', 'sentiment_thread', 'flair_loader_thread',
                'lexicon_loader_thread', 'model_loader_thread', 'import_thread'
            ]
            if hasattr(self, 'wordcloud_service'):
                try:
                    self.wordcloud_service.stop_generation()
                except Exception as e:
                    logger.warning(f"Error stopping wordcloud service: {e}")            
            if hasattr(self, 'sentiment_service'):
                try:
                    self.sentiment_service.cancel_analysis()
                except Exception as e:
                    logger.warning(f"Error canceling sentiment analysis: {e}")
            for thread_name in special_thread_names:
                thread = getattr(self, thread_name, None)
                if thread and isinstance(thread, QThread) and thread.isRunning():
                    logger.info(f"Found running thread: {thread_name}")
                    try:
                        thread.objectName()
                        special_threads.append((thread_name, thread))
                    except RuntimeError:
                        logger.warning(f"Thread {thread_name} has invalid C++ object reference, skipping")
            ...
            if hasattr(self, 'topic_tab'):
                ...
                for attr_name, thread_desc in topic_threads:
            ...
            for thread_name, thread in special_threads:
                ...
                try:
                    ...
                    if thread.wait(800):
                        ...
                        logger.warning(f"Thread {thread_name} not responding, terminating...")
                        thread.terminate()
                        if thread.wait(500):
                            stopped_special += 1
                            ...
                            failed_special += 1
                            logger.error(f"Failed to terminate thread {thread_name}")
                except Exception as e:
                    logger.error(f"Error stopping thread {thread_name}: {e}")
                    failed_special += 1
            result = None
            if hasattr(self, 'thread_manager'):
                logger.info("Using ThreadManager to stop remaining threads")
                try:
                    result = self.thread_manager.stop_all_threads(wait_timeout=800)
                except Exception as e:
                    logger.error(f"Error in ThreadManager.stop_all_threads: {e}")
            report = "Threads stopped successfully"
            if result:
                report = f"Threads: {result.get('total', 0)} • Stopped: {result.get('stopped', 0)} • Failed: {result.get('failed', 0)} • Remaining: {result.get('remaining', 0)} • Special: {stopped_special}/{len(special_threads)}"
    ...
    def generate_wordcloud(self):
        ...
        if selected_colormap.startswith("custom_"):
            print(f"Selected custom palette: {selected_colormap}")
            if selected_colormap in custom_palettes:
                print(f"Colors in palette: {custom_palettes[selected_colormap]}")
            else:
                print(f"Warning: Selected palette {selected_colormap} not found in available palettes")
        params = {
