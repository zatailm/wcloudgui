...
class WordCloudGenerator(QMainWindow):
    ...
    def initUI(self):
        ...
        self.file_label = QTextEdit('..... no file selected ..... :(', self)
        self.file_label.setFrameStyle(QFrame.Panel | QFrame.Sunken)
        self.file_label.setStyleSheet("QLineEdit { border: 1px solid grey; padding: 5px; }")
        self.file_label.setFixedHeight(30)  # Set fixed height for the label
        self.file_label.setReadOnly(True)
        layout.addWidget(self.file_label, 0, 0, 1, 3)

        self.upload_button = QPushButton('Select File', self)
        self.upload_button.setFixedHeight(30)  # Set fixed height for the button
        ...
        self.progress_bar.setFixedHeight(10)  # Set fixed height for the progress bar
        ...
        layout.addWidget(self.stopword_label, 3, 0, 1, 3)
        ...
        layout.addWidget(self.stopword_entry, 4, 0, 1, 3)

        self.color_theme_label = QLabel('Color Theme:', self)
        layout.addWidget(self.color_theme_label, 5, 0, 1, 1)
        ...
        layout.addWidget(self.color_theme, 5, 1, 1, 2)

        self.custom_palette_button = QPushButton('Custom Palette', self)
        self.custom_palette_button.setFixedHeight(30)
        self.custom_palette_button.setFixedWidth(150)  # Perkecil lebar tombol
        ...
        layout.addWidget(self.custom_palette_button, 6, 0, 1, 3, Qt.AlignRight)

        self.font_choice_label = QLabel('Font Choice:', self)
        layout.addWidget(self.font_choice_label, 7, 0, 1, 1)
        ...
        layout.addWidget(self.font_choice, 7, 1, 1, 2)

        self.min_font_size_label = QLabel('Minimum Font Size:', self)
        layout.addWidget(self.min_font_size_label, 8, 0, 1, 1)
        self.min_font_size_entry = QSpinBox(self)
        self.min_font_size_entry.setValue(10)
        layout.addWidget(self.min_font_size_entry, 8, 1, 1, 2)

        self.max_words_label = QLabel('Maximum Words:', self)
        layout.addWidget(self.max_words_label, 9, 0, 1, 1)
        self.max_words_entry = QSpinBox(self)
        self.max_words_entry.setMaximum(10000)
        self.max_words_entry.setValue(200)  # Set the value after setting the maximum
        layout.addWidget(self.max_words_entry, 9, 1, 1, 2)

        self.bg_color_label = QLabel('Background Color:', self)
        layout.addWidget(self.bg_color_label, 10, 0, 1, 1)
        ...
        layout.addWidget(self.bg_color, 10, 1, 1, 2)

        self.mask_label = QLabel('Mask Image:', self)
        layout.addWidget(self.mask_label, 11, 0, 1, 1)
        self.mask_button = QPushButton('Mask Image', self)
        self.mask_button.setFixedHeight(30)  # Set fixed height for the button
        ...
        layout.addWidget(self.mask_button, 11, 1, 1, 1)

        self.reset_mask_button = QPushButton('Remove Mask', self)
        self.reset_mask_button.setFixedHeight(30)  # Set fixed height for the button
        self.reset_mask_button.setToolTip('Remove the selected mask image')
        self.reset_mask_button.clicked.connect(self.reset_mask)
        layout.addWidget(self.reset_mask_button, 11, 2, 1, 1)

        self.mask_path_label = QLabel('Mask: No mask (rectangle)', self)
        layout.addWidget(self.mask_path_label, 12, 0, 1, 3)

        self.buat_wordcloud_button = QPushButton('Generate WordCloud', self)
        self.buat_wordcloud_button.setFixedHeight(30)  # Set fixed height for the button
        ...
        layout.addWidget(self.buat_wordcloud_button, 13, 0, 1, 3)

        self.statistik_button = QPushButton('Statistics', self)
        self.statistik_button.setFixedHeight(30)  # Set fixed height for the button
        ...
        layout.addWidget(self.statistik_button, 14, 0, 1, 3)

        self.simpan_button = QPushButton('Save WordCloud', self)
        self.simpan_button.setFixedHeight(30)  # Set fixed height for the button
        ...
        layout.addWidget(self.simpan_button, 15, 0, 1, 3)

        self.about_button = QPushButton('About', self)
        self.about_button.setFixedSize(150, 30)  # Set fixed size for the button
        self.about_button.setToolTip('Show information about the application')
        self.about_button.clicked.connect(self.show_about)
        layout.addWidget(self.about_button, 16, 0, 1, 3)

        self.quit_button = QPushButton('Quit', self)
        self.quit_button.setFixedHeight(30)  # Set fixed height for the button
        self.quit_button.setToolTip('Quit WCGen :(')
        self.quit_button.clicked.connect(self.close)
        layout.addWidget(self.quit_button, 16, 2, 1, 1)
    ...
    def closeEvent(self, event):
        ...
        if reply == QMessageBox.Yes:
            # Close all open dialogs or windows
    ...
    def pilih_mask(self):
        ...
        if mask_path:
            ...
            self.mask_path_label.setText(f"Mask Path: {mask_path}")

    def reset_mask(self):
        self.mask_path = ""
        self.mask_path_label.setText("Mask Path: (None)")
    ...
    def tampilkan_statistik(self):
        ...
        # Sort word counts from highest to lowest
        sorted_word_counts = sorted(word_counts.items(), key=lambda item: item[1], reverse=True)
        
        # Create a custom dialog for displaying statistics
        dialog = QDialog(self)
        ...
        # Create a table to display word frequencies
        table = QTableWidget()
