...
class WordCloudGenerator(QMainWindow):
    def __init__(self):
        ...
        self.thread_manager.add_thread(self.startup_thread)
    ...
    def handle_translation_failure(self, message):
    ...
    def stop_all_processes(self):
        ...
        self.thread_manager.cleanup_finished()
    ...
    def cleanup(self):
        ...
        try:
            ...
            print(f"Error clearing thread pool: {e}")
            ...
            for thread in remaining_threads:
                try:
                    if thread and thread.isRunning():
                        thread.requestInterruption()
                        thread.quit()
                        if not thread.wait(THREAD_TIMEOUT["TERMINATION"]):
                            thread.terminate()
                            thread.wait(THREAD_TIMEOUT["TERMINATION"])
                except Exception as e:
        ...
        try:
            ...
            print(f"Error in widget cleanup: {e}")

        try:
            if hasattr(self, 'flair_classifier'):
                del self.flair_classifier
            if hasattr(self, 'flair_classifier_cuslang'):
                del self.flair_classifier_cuslang
            if hasattr(self, 'textblob_analyzer'):
                del self.textblob_analyzer
        except Exception as e:
            print(f"Error cleaning up analyzers: {e}")
    ...
    def show_sentiment_analysis(
        ...
        score_type = ""
        if analysis_mode in ["VADER", "VADER (Custom Lexicon)"]:
            score_type = "Compound Score"
        elif analysis_mode in ["TextBlob", "TextBlob (Custom Lexicon)"]:
            score_type = "Polarity Score"
        elif analysis_mode in ["Flair", "Flair (Custom Model)"]:
        ...
        <table border="1" cellspacing="0" cellpadding="2" width="100%">
            ...
            <tr><td>{score_type}</td><td>{compound_score:.2f}</td></tr>
        """

        if analysis_mode in ["TextBlob", "TextBlob (Custom Lexicon)"]:
            sentiment_result += f'<tr><td>Subjectivity</td><td>{subjectivity:.2f}</td></tr>'
