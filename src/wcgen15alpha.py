...
class FileLoaderThread(QThread):
    ...
    def extract_text_from_excel(self, excel_path):
        """Extract text from an Excel file by concatenating all sheet data."""
        try:
...
class CustomFileLoaderThread(QThread):
    file_loaded = Signal(object, bool)  # Changed to emit object instead of path
    ...
    def run(self):
        try:
            if self.file_type == 'lexicon':
                ...
                # Load model and emit the instance directly
...
class CustomVaderSentimentIntensityAnalyzer(SentimentIntensityAnalyzer):
    def __init__(self, lexicon_file="vader_lexicon.txt", custom_lexicon_file=None):
        super().__init__(lexicon_file)  # Initialize with the default lexicon

        if custom_lexicon_file and os.path.exists(custom_lexicon_file):
            self.load_custom_lexicon(custom_lexicon_file)  # Load custom lexicon if provided
...
def notify_offline_mode(message):
    QMessageBox.warning(None, "Offline Mode", message)
def is_connected():
    try:
    ...
    def run(self):
        ...
        try:
            ...
            if self.sentiment_mode in ["VADER", "Flair", "TextBlob (Non-English)"]:
                try:
                    detected_lang = langdetect.detect(self.text_data)
            ...
            if needs_translation:
                ...
                if translated_text:
                    ...
                    result["sentiment_label"] = "Translation failed"
                    self.sentiment_analyzed.emit(result)
                    return

            if self.sentiment_mode == "TextBlob":
                if langdetect.detect(self.text_data) != "en" and not is_connected():
                    self.offline_warning.emit("TextBlob requires English text in offline mode")
                    return
                result.update(self.analyze_textblob(text_to_analyze))

            elif self.sentiment_mode == "TextBlob (Non-English)":
                result.update(self.analyze_textblob(text_to_analyze))
                ...
                if not self.flair_classifier_cuslang:
                    result["sentiment_label"] = "Error: Model not loaded"
                    self.sentiment_analyzed.emit(result)
                    return
                ...
                try:
                    ...
                    result["sentiment_label"] = f"Model Error: {str(e)}"

            else:
                result["sentiment_label"] = "Invalid analysis mode"

            self.sentiment_analyzed.emit(result)

        except Exception as e:
            result["sentiment_label"] = f"Critical Error: {str(e)}"
            self.sentiment_analyzed.emit(result)
...
class WordCloudGenerator(QMainWindow):
    ...
    def initUI(self):
        ...
        self.color_theme.setFixedSize(240,30)
        ...
        self.bg_color.setFixedSize(240,30)
        ...
        self.mask_button.setFixedSize(155, 30) 
        ...
        layout.addWidget(self.mask_button, 12, 1, 1, 1, Qt.AlignLeft)

        self.reset_mask_button = QPushButton('Remove Mask', self)
        self.reset_mask_button.setFixedSize(155, 30)
        ...
        layout.addWidget(self.sentiment_button, 16, 0, 1, 3)
        ...
        self.sentiment_mode_combo.setFixedSize(285,30)
        ...
        self.custom_lexicon_button.setFixedSize(155, 30)
        ...
        layout.addWidget(self.custom_lexicon_button, 19, 1, 1, 1, Qt.AlignLeft)

        self.custom_model_button = QPushButton('Load Model', self)
        self.custom_model_button.setFixedSize(155, 30)
        ...
        layout.addWidget(self.model_progress_bar, 20, 1, 1, 1, Qt.AlignRight)
        ...
        layout.addWidget(self.statistik_button, 21, 0, 1, 3)
        ...
        layout.addWidget(self.simpan_button, 22, 0, 1, 3)
        ...
        layout.addWidget(self.about_button, 23, 0, 1, 3)

        self.panic_button = QPushButton('Panic Button', self)
        self.panic_button.setFixedSize(150, 30)
        self.panic_button.setToolTip('Stop all ongoing processes')
        self.panic_button.clicked.connect(self.stop_all_processes)
        layout.addWidget(self.panic_button, 23, 1, 1, 1) 

        self.quit_button = QPushButton('Quit', self)
        self.quit_button.setFixedHeight(30) 
        self.quit_button.setToolTip('Quit WCGen :(')
        self.quit_button.clicked.connect(self.close)
        layout.addWidget(self.quit_button, 23, 2, 1, 1)
    ...
    def analyze_sentiment(self):
        ...
        self.sentiment_thread = SentimentAnalysisThread(
            self.text_data, self.sentiment_mode, self.vader_analyzer, self.flair_classifier, self.flair_classifier_cuslang
        )
    ...
    def change_sentiment_mode(self, mode):
        ...
        if mode == "VADER (Custom Lexicon)" and self.custom_lexicon_path is None:
            QMessageBox.warning(self, "Lexicon Not Loaded", "Please load a custom lexicon file for VADER sentiment analysis.")
        elif mode == "Flair (Custom Model)" and self.custom_model_path is None:
            QMessageBox.warning(self, "Model Not Loaded", "Please load a custom model file for Flair sentiment analysis.")
    ...
    def load_custom_model(self):
        options = QFileDialog.Options()
        model_path, _ = QFileDialog.getOpenFileName(self, "Select Custom Model File", "", "Model Files (*.pt)", options=options)
        if model_path:
            self.model_progress_bar.setVisible(True)
            self.model_progress_bar.setRange(0, 0) 
            self.custom_model_button.setEnabled(False) 
