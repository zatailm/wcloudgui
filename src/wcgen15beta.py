...
#        layout.addWidget(self.title_font_weight, 10, 5, 1, 1)
    ...
    def analyze_sentiment(self):
        ...
        if hasattr(self, "sentiment_thread") and self.sentiment_thread.isRunning():
            QMessageBox.warning(self, "Processing", "A sentiment analysis is already in progress. Please wait for it to finish.")
            return

        # Proceed with analysis
        self.progress_bar.show()
        self.sentiment_thread = SentimentAnalysisThread(
            self.text_data, self.sentiment_mode, self.vader_analyzer, classifier
        )
